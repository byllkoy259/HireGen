...
export interface AdminLayoutProps {
    ...
    pageTitle?: string;
    /** Dòng phụ dưới tiêu đề (mặc định: ngày hôm nay · System Admin) */
    pageSubtitle?: string;
    /**
     ...
     * Nếu không truyền, sidebar dùng nav mặc định của Admin module.
     */
...
const ADMIN_NAV_SECTIONS: NavSection[] = [
    {
        ...
        items: [
            { icon: 'manage_accounts', label: 'Quản lý HR', href: '/admin' },
            { icon: 'business', label: 'Đối tác', href: '/admin/partners' },
        ],
    },
    {
        title: 'CÀI ĐẶT',
    ...
    useEffect(() => {
        const fetchUser = async () => {
            try {
                ...
                const name = res.data.full_name || res.data.username || res.data.name || 'Quản trị viên';
                ...
                if (token) {
                    try {
                        ...
                        const fallbackName = payload.name || payload.username || payload.email?.split('@')[0] || 'Quản trị viên';
                        setUserData({
                            name: fallbackName,
                            email: payload.email || '',
                    ...
                    setUserData({ name: 'Quản trị viên', email: 'admin@hiregen.com', displayInitial: 'A', avatarUrl: '' });
                }
            }
        };
        fetchUser();
                ...
                <header className={styles.topbar}>
                    <div>
                        ...
                        <p className={styles.headerSub}>
                            {pageSubtitle ?? `${todayStr} · System Admin`}
                        </p>
                    </div>
